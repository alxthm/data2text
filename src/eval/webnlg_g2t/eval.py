# eval script for  2020 WebNLG challenge
# from https://github.com/WebNLG/GenerationEval/blob/master/eval.py
#
# requires text files for
#   - hypothesis: model predictions, true-cased and detokenized (one line per graph)
#   - reference: ground truth, one line per graph (multiple files if multiple lexicalizations)
#
# reference files are generated at dataset creation,
# following https://gitlab.com/webnlg/corpus-reader/-/blob/master/generate_references.py
...
def parse(refs_path, hyps_path, num_refs, lng="en"):
    ...
    for i in range(num_refs):
        fname = refs_path.replace("<id>", str(i)) if num_refs > 1 else refs_path
...
if __name__ == "__main__":
    ...
    argParser.add_argument(
        "-R", "--reference", help="reference translation", required=True
    )
    argParser.add_argument(
        "-H", "--hypothesis", help="hypothesis translation", required=True
    )
    argParser.add_argument(
        "-lng", "--language", help="evaluated language", default="en"
    )
    argParser.add_argument(
        "-nr", "--num_refs", help="number of references", type=int, default=4
    )
    argParser.add_argument(
        "-m",
        "--metrics",
        help="evaluation metrics to be computed",
        default="bleu,meteor,ter,chrf++,bert,bleurt",
    )
    argParser.add_argument(
        "-nc",
        "--ncorder",
        help="chrF metric: character n-gram order (default=6)",
        type=int,
        default=6,
    )
    argParser.add_argument(
        "-nw",
        "--nworder",
        help="chrF metric: word n-gram order (default=2)",
        type=int,
        default=2,
    )
    argParser.add_argument(
        "-b",
        "--beta",
        help="chrF metric: beta parameter (default=2)",
        type=float,
        default=2.0,
    )
