...
class T2G(nn.Module):
    ...
    def forward(self, batch):
        ...
        Returns:
            output of shape (bs, max_num_ent, max_num_ent, tot_num_rel)
                log probabilities of the relation type between entities found
                in the sentences. `bs` is batch size, `max_num_ent` is the maximum
                number of entities found in the batch sequences, and `tot_num_rel`
                is the total number of possible relation types.
